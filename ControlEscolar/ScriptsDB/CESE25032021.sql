...
CREATE TABLE IF NOT EXISTS `SIE`.`Alumno` (
  ...
  `Foto` INT NULL,
  `GrupoIndigena_idBecas` INT NOT NULL,
  ...
  INDEX `fk_Alumno_GrupoIndigena1_idx` (`GrupoIndigena_idBecas` ASC),
  INDEX `fk_Alumno_Discapacidad1_idx` (`Discapacidad_Id` ASC),
  ...
  CONSTRAINT `fk_Alumno_Becas1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Alumno_GrupoIndigena1`
    FOREIGN KEY (`GrupoIndigena_idBecas`)
    REFERENCES `SIE`.`GrupoIndigena` (`idBecas`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Alumno_Discapacidad1`
    FOREIGN KEY (`Discapacidad_Id`)
    REFERENCES `SIE`.`Discapacidad` (`Id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `SIE`.`Periodos` (
  ...
  `Kardex_idKardex` INT NULL,
  `Estudios_idEstudios` INT NULL,
...
CREATE TABLE IF NOT EXISTS `SIE`.`Alumno-Empresa` (
  `idAlumno-Empresa` INT NOT NULL AUTO_INCREMENT,
  `Desde` DATE NULL,
  `Hasta` DATE NULL,
  `Alumno_idAlumno` INT NOT NULL,
  `Empresa_idEmpresa` INT NOT NULL,
